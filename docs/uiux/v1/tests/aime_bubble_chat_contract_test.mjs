...
assert.match(css, /\.aime-pet-trigger \{[\s\S]*?width:\s*172px;[\s\S]*?height:\s*172px;/, "宠物独立入口命中尺寸必须与 App 端 86pt 宠物一致");
assert.match(css, /\.aime-panel \{[\s\S]*?height:\s*60%;[\s\S]*?min-height:\s*720px;[\s\S]*?border-radius:\s*31px 31px 0 0;/, "Aime 初始面板必须与 App 端 max(360pt, 60%) 半屏高度一致");
assert.match(css, /\.aime-panel\.is-full \{[\s\S]*?height:\s*100%;[\s\S]*?max-height:\s*100%;/, "完整对话状态必须占满画布高度");
