...
assert.match(css, /\.aime-fab:not\(\.is-collapsed\) \.aime-pet-visual \{\s*transform:\s*translateX\(46px\);/, "Expanded pet must sit close to the canvas right edge");
assert.match(css, /\.aime-fab:not\(\.is-collapsed\) \.aime-fab-bubble \{\s*transform:\s*translate\(39px, -35px\);/, "Expanded bubble must keep a clear gap above the three-circle trail");
assert.match(css, /\.aime-fab:not\(\.is-collapsed\) \.aime-fab-arrow \{\s*transform:\s*translate\(66px, -27px\) scaleX\(-1\);/, "Expanded mirrored bubble trail must stay fully visible between the bubble and head");
...
assert.match(js, /navigationTop - fabRect\.height - 24/, "Dragging must stay above the bottom navigation");
assert.match(css, /--aime-expanded-width:\s*264px;[\s\S]*?--aime-collapsed-width:\s*172px;/, "AIMe must expose stable target widths for one-step right alignment");
assert.match(js, /const targetWidth = getAimeTargetWidth\(aimeCollapsed\);\s*const targetLeft = aimeCollapsed \? frameRect\.right - targetWidth : frameRect\.right - targetWidth - 24;[\s\S]*?renderAimePosition\(Math\.max\(bounds\.minLeft, targetLeft\), top\);/, "AIMe must calculate stable collapsed and expanded right-side targets without a second correction");
