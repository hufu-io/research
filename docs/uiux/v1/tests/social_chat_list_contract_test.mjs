...
assert.match(socialSection, /data-social-filter="other">其他/, "必须提供其他会话分类");
assert.match(socialSection, /data-social-filter="work">工作/, "必须提供工作会话分类");
assert.match(socialSection, /data-chat-category="other"/, "必须包含其他分类会话");
assert.match(socialSection, /data-chat-category="work"/, "必须包含工作分类会话");
...
assert.match(css, /\.social-filter-tabs button \{[\s\S]*?min-height:\s*68px;/, "分类触控目标必须达到 44px");
assert.match(css, /\.social-filter-tabs \{[\s\S]*?min-height:\s*72px;[\s\S]*?padding:\s*0;/, "分类栏视觉高度必须保持紧凑");
